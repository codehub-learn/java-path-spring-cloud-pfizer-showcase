...
public abstract class AbstractController<E, R> extends AbstractLogComponent {
	...
	public ResponseEntity<ApiResponse<R>> create(@Valid @RequestBody final R resource) {
		return new ResponseEntity<>(ApiResponse.<R>builder().data(getMapper().toResource(
				getBaseService().save(getMapper().toDomain(resource)))).build(), WebUtils.getNoCacheHeaders(),
									HttpStatus.CREATED);
	}
